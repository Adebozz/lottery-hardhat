...
describe("Lottery Contract", function () {
  ...
  it("allows multiple accounts to enter", async function () {
    await lottery.enter({ value: ethers.parseEther("0.02") });
    await lottery.connect(accounts[1]).enter({ value: ethers.parseEther("0.02") });
    await lottery.connect(accounts[2]).enter({ value: ethers.parseEther("0.02") });
  ...
  it("requires a minimum amount of ether to enter", async function () {
    await expect(
      lottery.enter({ value: ethers.parseEther("0.001") })
    ).to.be.reverted;
  });
  ...
  it("sends money to the winner and resets the players array", async function () {
    ...
    expect(finalBalance - initialBalance).to.be.greaterThan(ethers.parseEther("1.8"));
